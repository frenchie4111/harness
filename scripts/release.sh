...
if ! gh repo set-default --view >/dev/null 2>&1; then
  fail "gh has no default repo for this directory. Run: gh repo set-default ness-dev/ness"
fi
...
echo
echo "${BOLD}Ready to prepare Ness v${VERSION}${RESET}"
...
echo "  https://github.com/ness-dev/ness/actions"
echo
...
for (const f of files) {
  ...
  content = content.replace(/(?:Harness|Ness)-\d+\.\d+\.\d+/g, \`Ness-\${v}\`);
...
- **Apple Silicon:** \`Ness-${VERSION}-arm64.dmg\`
- **Intel:** \`Ness-${VERSION}.dmg\`
...
Auto-generated by \`scripts/release.sh\` for Ness ${TAG}.

This PR contains the version bump, README link updates, \`site/public/releases.html\` entry, and \`release-notes/${TAG}.md\` for the release. Once CI passes and this PR is merged, the script will tag the new \`main\` HEAD as \`${TAG}\`. The tag push then fires [\`release.yml\`](https://github.com/ness-dev/ness/blob/main/.github/workflows/release.yml), which builds, signs, notarizes, and uploads artifacts to the GitHub release.
...
echo "    https://github.com/ness-dev/ness/actions/workflows/release.yml"
echo "  The GitHub release will appear at:"
echo "    https://github.com/ness-dev/ness/releases/tag/${TAG}"
